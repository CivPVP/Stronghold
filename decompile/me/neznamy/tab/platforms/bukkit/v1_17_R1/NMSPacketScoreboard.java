...
public class NMSPacketScoreboard extends SafeScoreboard<BukkitTabPlayer> {
   ...
   public void updateObjective(@NonNull SafeScoreboard.Objective objective) {
      ...
      obj.setRenderType(EnumScoreboardHealthDisplay.valueOf(objective.getHealthDisplay().name()));
   ...
   public void setScore(@NonNull SafeScoreboard.Score score) {
      ...
      this.sendPacket(new PacketPlayOutScoreboardScore(Action.a, score.getObjective().getName(), score.getHolder(), score.getValue()));
   }
   ...
   public void removeScore(@NonNull SafeScoreboard.Score score) {
      ...
      this.sendPacket(new PacketPlayOutScoreboardScore(Action.b, score.getObjective().getName(), score.getHolder(), score.getValue()));
   }
   ...
   public Object onPacketSend(@NonNull Object packet) {
      try {
         ...
         if (packet instanceof PacketPlayOutScoreboardDisplayObjective) {
            TAB.getInstance()
               .getFeatureManager()
               .onDisplayObjective(this.player, ((PacketPlayOutScoreboardDisplayObjective)packet).b(), ((PacketPlayOutScoreboardDisplayObjective)packet).c());
         }

         if (packet instanceof PacketPlayOutScoreboardObjective) {
            TAB.getInstance()
               .getFeatureManager()
               .onObjective(this.player, ((PacketPlayOutScoreboardObjective)packet).d(), ((PacketPlayOutScoreboardObjective)packet).b());
         }

         if (packet instanceof PacketPlayOutScoreboardTeam) {
            int action = this.getMethod((PacketPlayOutScoreboardTeam)packet);
            if (action != 2) {
   ...
   private void sendPacket(@NotNull Packet<?> packet) {
      ((CraftPlayer)this.player.getPlayer()).getHandle().b.sendPacket(packet);
   }
}
