package me.neznamy.tab.platforms.bukkit.v1_20_R2;
   ...
   public Object onPacketSend(@NonNull Object packet) {
      try {
         ...
         if (packet instanceof PacketPlayOutScoreboardDisplayObjective display) {
            TAB.getInstance().getFeatureManager().onDisplayObjective(this.player, display.a().ordinal(), display.d());
         }

         if (packet instanceof PacketPlayOutScoreboardObjective objective) {
            TAB.getInstance().getFeatureManager().onObjective(this.player, objective.e(), objective.a());
         }

         if (packet instanceof PacketPlayOutScoreboardTeam team) {
            int action = this.getMethod(team);
            if (action != 2) {
               TeamPacket_PLAYERS.set(packet, this.onTeamPacket(action, team.e(), team.f() == null ? Collections.emptyList() : team.f()));
            }
         }
