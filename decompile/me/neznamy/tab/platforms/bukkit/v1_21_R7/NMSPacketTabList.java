package me.neznamy.tab.platforms.bukkit.v1_21_R7;
...
public class NMSPacketTabList extends TrackedTabList<BukkitTabPlayer> {
   ...
   private static final a UPDATE_LIST_ORDER = a.valueOf("UPDATE_LIST_ORDER");
   private static final a UPDATE_HAT = a.valueOf("UPDATE_HAT");
   ...
   private static final EnumSet<a> UPDATE_LIST_ORDER_SET = EnumSet.of(UPDATE_LIST_ORDER);
   private static final EnumSet<a> UPDATE_HAT_SET = EnumSet.of(UPDATE_HAT);
   ...
   public void updateDisplayName0(@NonNull UUID entry, @Nullable TabComponent displayName) {
      ...
      this.sendPacket(UPDATE_DISPLAY_NAME_SET, entry, "", null, false, 0, 0, displayName, 0, false);
   }
   ...
   public void updateLatency(@NonNull UUID entry, int latency) {
      ...
      this.sendPacket(UPDATE_LATENCY_SET, entry, "", null, false, latency, 0, null, 0, false);
   }
   ...
   public void updateGameMode(@NonNull UUID entry, int gameMode) {
      ...
      this.sendPacket(UPDATE_GAME_MODE_SET, entry, "", null, false, 0, gameMode, null, 0, false);
   }
   ...
   public void updateListed(@NonNull UUID entry, boolean listed) {
      ...
      this.sendPacket(UPDATE_LISTED_SET, entry, "", null, listed, 0, 0, null, 0, false);
   }
   ...
   public void updateListOrder(@NonNull UUID entry, int listOrder) {
      ...
      this.sendPacket(UPDATE_LIST_ORDER_SET, entry, "", null, false, 0, 0, null, listOrder, false);
   }
   ...
   public void updateHat(@NonNull UUID entry, boolean showHat) {
      ...
      this.sendPacket(UPDATE_HAT_SET, entry, "", null, false, 0, 0, null, 0, showHat);
   }
   ...
   public TabList.Skin getSkin() {
      Collection<Property> properties = ((CraftPlayer)this.player.getPlayer()).getProfile().properties().get("textures");
   ...
   public Object onPacketSend(@NonNull Object packet) {
      try {
         ...
         if (packet instanceof PacketPlayOutPlayerListHeaderFooter tablist) {
            ...
            if (tablist.b != this.header.convert() || tablist.c != this.footer.convert()) {
         ...
         if (packet instanceof ClientboundPlayerInfoUpdatePacket info) {
            ...
            for (b nmsData : info.e()) {
               ...
               if (actions.contains(ADD_PLAYER)) {
                  TAB.getInstance().getFeatureManager().onEntryAdd(this.player, profileId, nmsData.b().name());
               }

               updatedList.add(
                  rewriteEntry
                     ? new b(profileId, nmsData.b(), nmsData.c(), latency, EnumGamemode.a(gameMode), displayName, nmsData.g(), nmsData.h(), nmsData.i())
                     : nmsData
               );
            }
      ...
      try {
         ...
         PLAYERS.set(
            ...
            Collections.singletonList(
               new b(
                  id,
                  this.createProfile(id, name, skin),
                  listed,
                  latency,
   ...
   private void sendPacket(@NotNull Packet<?> packet) {
      ((CraftPlayer)this.player.getPlayer()).getHandle().g.b(packet);
   }
}
