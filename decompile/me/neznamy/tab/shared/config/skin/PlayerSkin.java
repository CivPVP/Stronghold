...
public class PlayerSkin extends SkinSource {
   ...
   public TabList.Skin download(@NotNull String input) {
      try {
         ...
         if (player != null) {
            TabList.Skin skin = player.getTabList().getSkin();
            if (skin != null) {
               TAB.getInstance()
                  .debug("Skipping downloading of PLAYER skin " + input + ", because such player is online. Taking skin from their profile instead.");
               return skin;
            }
         }

         long time = System.currentTimeMillis();
         JSONObject json = this.getResponse("https://api.ashcon.app/mojang/v2/user/" + input);
